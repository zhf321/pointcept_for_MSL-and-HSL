...
def build_optimizer(cfg, model, param_dicts=None, is_query=False):
    if param_dicts is None:
        if not is_query:
            ...
            for name, param in model.named_parameters():
                # 过滤出 query 相关的参数
                if 'backbone_q' in name or 'g_q' in name or 'h_q' in name or 'proto_sq' in name or 'h_q_' in name:
                    query_parameters.append(param)
            cfg.params = query_parameters
    else:
