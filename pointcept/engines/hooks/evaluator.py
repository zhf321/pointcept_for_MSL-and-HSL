...
@HOOKS.register_module()
class SemSegEvaluator_U(HookBase):
    ...
    def eval(self):
        # eval :  batch_size  应该是可以变大的
        self.trainer.logger.info(">>>>>>>>>>>>>>>> Start Evaluation >>>>>>>>>>>>>>>>")
        self.trainer.model.eval()
        self.trainer.classifier.eval()

        primitive_centers = self.trainer.classifier.weight.data
        print("Merging Primitives")

        cluster_pred = KMeans(n_clusters=self.trainer.cfg.data.num_classes,
                              n_init=10,
                              random_state=0).fit_predict(primitive_centers.cpu().numpy())

        '''Compute Class Centers'''  # 计算每一类的聚类中心
        centroids = torch.zeros((self.trainer.cfg.data.num_classes, self.trainer.cluster_cfg.feats_dim))
        for cluster_idx in range(self.trainer.cfg.data.num_classes):
        ...
        # #
        centroids = F.normalize(centroids, dim=1)
        classifier = SemSegEvaluator_U.get_fixclassifier(in_channel=self.trainer.cluster_cfg.feats_dim, 
                                       centroids_num=self.trainer.cfg.data.num_classes, 
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            for key in input_dict.keys():
                if isinstance(input_dict[key], torch.Tensor):
                    input_dict[key] = input_dict[key].cuda(non_blocking=True)
            with torch.no_grad():
                feats = self.trainer.model.backbone(input_dict)
                feats = F.normalize(feats, dim=1)

                scores = F.linear(F.normalize(feats), F.normalize(classifier.weight))
                preds = torch.argmax(scores, dim=1)
                # TODO: batch 里面每一个 inverse map 是独立的，要分开处理
                offset = input_dict["offset"]
                origin_offset = input_dict["inverse_offset"]
                unit_preds = []
                for i in range(len(offset)):
                    if i == 0:
                        unit_pred = preds[0:offset[i]]
                        unit_pred = unit_pred[input_dict["inverse"][0:origin_offset[i]].long()]
                        unit_preds.append(unit_pred)
                    else:
                        unit_pred = preds[offset[i - 1]: offset[i]]
                        unit_pred = unit_pred[input_dict["inverse"][origin_offset[i - 1]:origin_offset[i]].long()]
                        unit_preds.append(unit_pred)
                preds = torch.cat(unit_preds)
                # preds = preds[input_dict["inverse"].long()]
                origin_segment = input_dict["origin_segment"]
                all_preds.append(preds[origin_segment!=self.trainer.cfg.data.ignore_index]), 
                all_labels.append(origin_segment[[origin_segment!=self.trainer.cfg.data.ignore_index]])
        ...
        histogram = torch.bincount(sem_num * all_labels[mask] + all_preds[mask], 
                                   minlength=sem_num ** 2).reshape(sem_num, sem_num)
        '''Hungarian Matching'''
        row_ind, col_ind = linear_sum_assignment(histogram.max().cpu().numpy() - histogram.cpu().numpy())
        acc_class = histogram[row_ind, col_ind] / (histogram.sum(dim=1) + 1e-10)* 100.
 
        all_acc = histogram[row_ind, col_ind].sum() / histogram.sum()*100.
        m_acc = torch.nanmean(histogram[row_ind, col_ind] / histogram.sum(1))*100
        hist_new = torch.zeros((sem_num, sem_num))
        for idx in range(sem_num):
            hist_new[:, idx] = histogram[:, col_ind[idx]]

        '''Final Metrics'''
        tp = torch.diag(hist_new)
        fp = torch.sum(hist_new, 0) - tp
        fn = torch.sum(hist_new, 1) - tp
        ...
        if self.trainer.writer is not None:
            ...
            self.trainer.writer.add_scalar("val/allAcc", all_acc, current_epoch)
            for i in range(self.trainer.cfg.data.num_classes):
                tag = f"class/{i}-" + self.trainer.cfg.data.names[i]
                self.trainer.writer.add_scalar(tag, IoUs[i], current_epoch)
