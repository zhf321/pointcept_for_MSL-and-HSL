...
class TitanDataset(Dataset):
    def __init__(
        ...
        super(TitanDataset, self).__init__()
    ...
    def get_data(self, idx):
        # 这里 idx % len(self.data_list) :
        # 因为 idx 是 从 __len__() 函数中获取的，可能已经乘以了 loop 倍
    ...
    def __len__(self):
        # 这里 * self.loop 有两个作用：
        # 1：epoch 如果大于100 且是100 的整数倍，可以转化为，epoch 还是100， 数据多了 loop 倍
        # 2：如果有些数据增强需要对数据循环使用多次
